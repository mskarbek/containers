...
pushd ${TMP_DIR}
    if [ ${IMAGE_BOOTSTRAP} == "true" ]; then
        ...
        curl -L $(jq -r .[1].remote_url ${PWD_DIR}/files/versions.json | sed "s;VERSION;${VERSION};g") | tar xzv
    else
        ...
        curl -u "${REPOSITORY_USERNAME}:${REPOSITORY_PASSWORD}" -L $(jq -r .[1].local_url ${PWD_DIR}/files/versions.json | sed "s;VERSION;${VERSION};g" | sed "s;REPOSITORY_URL;${REPOSITORY_URL};" | sed "s;REPOSITORY_RAW_REPO;${REPOSITORY_RAW_REPO};") | tar xzv
    fi
popd
mv -v ${TMP_DIR}/fleet_v${VERSION}_linux/fleet ${CONTAINER_PATH}/usr/local/bin/fleet
mv -v ${TMP_DIR}/fleetctl_v${VERSION}_linux/fleetctl ${CONTAINER_PATH}/usr/local/bin/fleetctl
